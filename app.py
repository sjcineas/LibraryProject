...
@app.route('/all_cards/<int:foreign_key>', methods=['GET'])
def all_cards(foreign_key: int):
    ...
    if card:
        ...
        return jsonify(message="User has no cards registered."), 404


#-----------------------Nathan--------------------------

# Retrieve book by author name
@app.route('/book_by/<string:book_author>', methods=['GET'])
...
@app.route('/retrieve_isbn/<int:ISBN>', methods=['GET'])
def Books(ISBN: int):
...
# Admin Privileges - can only add book if admin
...
#-----------------------Michael--------------------------
# Create Shopping Cart and add to cart
...
# Display current user's Shopping Cart
